...
export default function Perfil() {
  ...
  async function loadProfileData() {
    ...
    try {
      ...
      setAvatarUrl(
        u.avatarUrl ||
          `https://ui-avatars.com/api/?name=${u.fullName}&background=0D8ABC&color=fff`
      );
  ...
  async function handleAddSkill() {
    ...
    try {
      ...
      const payload = {
        ...
        description: `Habilidade de ${skillName}`,
        hourlyRate: 1,
      ...
      Alert.alert('Erro', 'Falha ao criar habilidade.');
    }
  }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.card}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, marginBottom: 5 }}>
            ...
            <TouchableOpacity
              style={styles.iaBadge}
              onPress={() => setBio(gerarBioLocalmente(fullName, skills.map(s => s.name)))}
            >
        ...
        <View style={styles.card}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
            {categoriasSQL.map(c => (
              <TouchableOpacity
                key={c}
                style={[styles.chip, selectedCategory === c && styles.chipSelected]}
                onPress={() => setSelectedCategory(c)}
              >
                <Text style={[styles.chipText, selectedCategory === c && styles.chipTextSelected]}>{c}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <View style={styles.typeContainer}>
            <TouchableOpacity
              onPress={() => setIsOffering(true)}
              style={[styles.typeOption, isOffering && { backgroundColor: '#E8F5E9', borderColor: '#4CAF50' }]}
            >
              <Text style={{ color: isOffering ? '#2E7D32' : '#555' }}>Ensinar</Text>
            </TouchableOpacity>

            <TouchableOpacity
              onPress={() => setIsOffering(false)}
              style={[styles.typeOption, !isOffering && { backgroundColor: '#E3F2FD', borderColor: '#000080' }]}
            >
              <Text style={{ color: !isOffering ? '#1565C0' : '#555' }}>Aprender</Text>
            </TouchableOpacity>
          </View>
          ...
          {skills.map(s => (
            <View key={s.id} style={styles.skillItem}>
              ...
              <TouchableOpacity onPress={() => handleDeleteSkill(s.id)}>
                <Feather name="trash-2" color="red" size={18} />
              </TouchableOpacity>
            </View>
          ))}
        </View>
        ...
        <TouchableOpacity style={styles.logoutButton} onPress={signOut}>
          <Text style={{ color: '#D32F2F' }}>Sair</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
